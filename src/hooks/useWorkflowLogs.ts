        ...
        const briefsWithDetails = await Promise.all(
          briefsData.map(async (brief) => {
            try {
              ...
              const stageMap = (conversations || []).reduce((acc: any, conv: any) => {
                ...
                if (!acc[stepId]) {
                  acc[stepId] = {
                    ...
                    agents: new Set()  // Initialize an empty Set for agents
                  };
                }
                
                // Add agent to the Set if it exists
                if (conv.agents) {
                  acc[stepId].agents.add(conv.agents.name);
                }
              ...
              // Convert stages to array and handle the agents Set
              const stages = Object.values(stageMap).map((stage: any) => ({
                ...stage,
                agents: stage.agents ? Array.from(stage.agents) : []
              }));
